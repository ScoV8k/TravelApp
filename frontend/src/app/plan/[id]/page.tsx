...
interface GeneratedPlan {
    ...
    emergency_contacts: EmergencyContact[] | null; // Dodano pole
}
...
function TripMapComponent({ locations, apiKey, selectedLocation }: TripMapComponentProps) {
    ...
    if (loadError) {
        return <div className="text-red-500 p-4 bg-red-100 rounded-md">Failed to load Google Maps.</div>;
    }
    ...
    if (!isLoaded) {
        return (
            <div className="flex items-center justify-center" style={mapContainerStyle}>
                ...
                <p className="ml-2 text-slate-600">Loading map...</p>
            </div>
        );
    }
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            if (!response.ok) {
                ...
                const errorMessage = typeof errorBody.detail === 'string' ? errorBody.detail : `Error fetching data (status: ${response.status})`;
            ...
            if (attempt < maxRetries - 1) {
                ...
                throw new Error(`Failed to fetch the plan after ${maxRetries} attempts. Last error: ${(lastError as Error).message}`);
            }
        }
    }
    throw lastError || new Error("Unknown error while fetching data after multiple attempts.");
}


const getActivityIcon = (type: string | null) => {
    const typeLower = type?.toLowerCase() || ""
    if (typeLower.includes("eat") || typeLower.includes("food") || typeLower.includes("restaurant") || typeLower.includes("dinner") || typeLower.includes("lunch") || typeLower.includes("breakfast")) return <Utensils className="w-4 h-4 mr-1.5 text-orange-500 shrink-0" />;
    if (typeLower.includes("hike") || typeLower.includes("nature") || typeLower.includes("park") || typeLower.includes("walk")) return <MountainSnow className="w-4 h-4 mr-1.5 text-green-500 shrink-0" />;
    if (typeLower.includes("museum") || typeLower.includes("landmark") || typeLower.includes("historical") || typeLower.includes("sightseeing") || typeLower.includes("attraction")) return <Landmark className="w-4 h-4 mr-1.5 text-purple-500 shrink-0" />;
    if (typeLower.includes("tour") || typeLower.includes("event") || typeLower.includes("ticket")) return <Ticket className="w-4 h-4 mr-1.5 text-yellow-500 shrink-0" />;
    if (typeLower.includes("shop") || typeLower.includes("shopping")) return <ShoppingBag className="w-4 h-4 mr-1.5 text-pink-500 shrink-0" />;
    if (typeLower.includes("hotel") || typeLower.includes("sleep") || typeLower.includes("accommodation")) return <BedDouble className="w-4 h-4 mr-1.5 text-teal-500 shrink-0" />;
...
export default function PlanPage() {
        ...
        const checkTripStatus = async () => {
            ...
            try {
                ...
                if (data.status === "planned") {
                    ...
                    if (!planRes.ok) throw new Error("Failed to fetch trip plan");
                    const planData = await planRes.json();
                    setGeneratedPlan(planData.data);
                    console.log("HELLO: ", planData.data);
                }
            } catch (err: any) {
                console.error(err);
                setError(err.message || "Error fetching status or plan");
            }
        };
        checkTripStatus();
    }, [tripId]);
        ...
        if (tripId) {
            setTimeout(() => {
                const formattedName = tripId
                    ...
                    .join(' ');
                setDisplayName(`Plan for: ${formattedName}`);
    ...
    if (!tripId) {
        setError("No trip ID. Cannot proceed.");
        setLoading(false);
        return;
    }
    setLoading(true);
    ...
    try {
        ...
        setError(err.message || "Failed to generate the plan after several attempts.");
    ...
    const handleGeneratePlanClick = () => {
            if (!tripId) {
                setError("No trip ID in URL. Cannot generate plan.");
                return;
            }
            setError(null);
    ...
    if (tripStatus === null) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen p-6 bg-slate-50">
                ...
                <p className="text-lg text-slate-700">Checking trip status...</p>
            </div>
        );
    }
                ...
                <div className="bg-white p-6 md:p-10 rounded-xl shadow-2xl max-w-md w-full">
                    ...
                    {!displayNameLoading && displayName && (
                        <p className="text-sm text-slate-600 mb-5 font-medium">
                            <span className="text-slate-500">For:</span> <strong className="text-blue-600">{displayName.replace("Plan for: ", "")}</strong>
                        </p>
                    )}
                    ...
                    >
                        {loading ? <Loader2 className="animate-spin w-5 h-5 mx-auto" /> : "Generate Plan"}
                    </button>
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen p-6 bg-slate-50">
                <Loader2 className="animate-spin w-12 h-12 text-blue-600 mb-4" />
                <p className="text-lg text-slate-700">Generating your trip plan...</p>
            </div>
        );
    }
    ...
    if (!generatedPlan) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen p-6 bg-slate-50">
                    ...
                    <p className="text-lg text-slate-700">No trip plan found.</p>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-100 via-sky-50 to-slate-100 py-8 md:py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-5xl mx-auto space-y-8">
                <header className="text-center mb-8 md:mb-12">
                    <h1 className="text-4xl md:text-5xl font-bold text-slate-800 mb-2">
                        {generatedPlan.trip_name || "Your Trip Plan"}
                    </h1>
                    ...
                    {generatedPlan.start_date && generatedPlan.end_date && (
                        <p className="text-md text-slate-500 mt-1">
                                ...
                                {generatedPlan.start_date} – {generatedPlan.end_date} ({generatedPlan.duration_days || 'N/A'} days)
                        </p>
                    )}
                </header>
                ...
                <section className="bg-white p-6 rounded-xl shadow-lg">
                    <div className="flex items-center mb-4">
                        ...
                        <h2 className="text-2xl md:text-3xl font-semibold text-slate-800">Daily Plan</h2>
                    </div>
                                        ...
                                        {day.accommodation && day.accommodation.hotel_name && (
                                            <div className="mb-6 p-4 bg-sky-50 rounded-lg border border-sky-200">
                                                <h3 className="font-semibold text-lg text-sky-800 mb-2 flex items-center">
                                                    <BedDouble className="w-5 h-5 mr-2 text-sky-600" /> Accommodation
                                                </h3>
                                                ...
                                                {day.accommodation.check_in && <p className="text-sm text-slate-500">Check-in: {day.accommodation.check_in}</p>}
                                            </div>
                                        )}

                                        {day.activities && day.activities.length > 0 && (
                                            <>
                                                <h3 className="font-semibold text-lg text-slate-700 mt-4 mb-3">Sights & Activities:</h3>
                                                            ...
                                                            <a key={i} href={mapLink} target="_blank" rel="noopener noreferrer"
                                                                ...
                                                                <div className="p-4">
                                                                    ...
                                                                    <span className="inline-flex items-center mt-3 text-sm text-blue-600 group-hover:underline">
                                                                        View on map <Navigation className="w-3.5 h-3.5 ml-1" />
                                                                    </span>
                                                                </div>
                                                            </a>
                                                        );
                    ...
                    ) : ( <p className="text-slate-500 text-center py-4">No detailed plan available.</p> )}
                </section>

                <section className="bg-white p-6 rounded-xl shadow-lg">
                        <div className="flex items-center mb-6">
                            <MapIconLucide className="w-7 h-7 mr-3 text-green-600" />
                            <h2 className="text-2xl md:text-3xl font-semibold text-slate-800">Trip Map</h2>
                        </div>
                        {googleMapsApiKey && validMapLocations.length > 0 ? (
                            ...
                            <div className="p-4 bg-slate-100 rounded-md text-center">
                                <p className="text-slate-600">
                                    {googleMapsApiKey ? 'No locations with valid coordinates to display.' : 'The Google Maps API key is not configured, so the map cannot be displayed.'}
                                </p>
                            </div>
                        )}
                        ...
                        {validMapLocations.length > 0 && (
                            <div className="mt-6">
                                <h3 className="text-lg font-semibold text-slate-700 mb-3">List of key locations:</h3>
                                <ul className="space-y-1 text-sm max-h-60 overflow-y-auto pr-2">
                                    {validMapLocations.map((loc, idx) => (
                                        ...
                                        >
                                            ...
                                            {loc.day && <span className="text-xs text-slate-400 ml-2">(Day {loc.day})</span>}
                ...
                {generatedPlan.general_notes && generatedPlan.general_notes.length > 0 && (
                    <section className="bg-white p-6 rounded-xl shadow-lg">
                        <div className="flex items-center mb-4"> <ClipboardList className="w-7 h-7 mr-3 text-purple-600" />
                            <h2 className="text-2xl md:text-3xl font-semibold text-slate-800">General Notes</h2>
                        </div>
                ...
                {/* --- NOWA SEKCJA EMERGENCY CONTACTS --- */}
                {generatedPlan.emergency_contacts && generatedPlan.emergency_contacts.length > 0 && (
                    <section className="bg-white p-6 rounded-xl shadow-lg">
                        <div className="flex items-center mb-4">
                            <LifeBuoy className="w-7 h-7 mr-3 text-red-600" />
                            <h2 className="text-2xl md:text-3xl font-semibold text-slate-800">Emergency Contacts</h2>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {generatedPlan.emergency_contacts.map((contact, idx) => (
                                <div key={idx} className="bg-slate-50 border border-slate-200 p-4 rounded-lg">
                                    ...
                                    {contact.type && (
                                        <p className="text-sm text-slate-500 mb-2">{contact.type}</p>
                                    )}
                                    {contact.phone && (
                ...
                >
                    {loading ? (
                        <span className="flex items-center"><Loader2 className="animate-spin w-5 h-5 mr-2" /> Generating...</span>
                    ) : (
                        "Regenerate"
                    )}
                </button>
                {/* <div className="w-full flex justify-center mt-4">
                    <button
                        onClick={() => {
                            ...
                            window.print(); // Tymczasowe rozwiązanie: eksport przez drukowanie
                        }}
                        className="bg-rose-600 hover:bg-rose-700 text-white px-6 py-3 rounded-lg text-sm font-medium shadow-md"
                    >
                        Export Plan to PDF
                    </button>
                </div> */}
            ...
            <footer className="text-center mt-12 py-6 border-t border-slate-200">
                <p className="text-sm text-slate-500">Trip plan generated. Have a great adventure!</p>
            </footer>
        </div>
    );
};